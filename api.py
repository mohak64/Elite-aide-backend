...
# #         template="""
# #         You are a highly advanced and accurate AI assistant for a grocery/essentials/official,unofficial meeting planner/schedule reminder mobile application. You need to extract the following information from the user's input string:
...
# #         Childcare
# #         Pet care
# #         Phone calls or video chats with friends/family
# #         Health and Wellness:
...
# #         Protocols you have to follow:
# #         Autocomplete feature should automatically complete the abbreviated/slang/incomplete/grammatically incorrect word if any present in user input string.
# #         If any information is missing from the user's input, ask for it minimum for the remaining necessary details and extract all details accordingly else return 'Not enough details provided'.
# #         Also return Completion date in 'YYYY-MM-DD' format ask for it from user but DO mandatory return CORRECT and ACCURATE date.
# #         If multiple tasks are being referred by user in input string then it should be able to handle it and give output smoothly without omitting any task.
# #         It must work like a professional chatbot which takes input from user and provide output in the instructed format where it asks for incomplete details one time again only and gives output in most efficient and professional way just like a personalised advanced AI chatbot (Jarvis) of 2024.
# #         """
# #     )
...
# #     output = llm(prompt.format(input=user_input))
# #     return output
...
# #     extracted_info_text = result.strip()
# #     extracted_info_dict = parse_extracted_info(extracted_info_text)

# #     if is_information_complete(extracted_info_dict):
# #         # Create task object and return success response
# #         task = Task(
...
# #     else:
# #         # Missing information, prompt user for details
# #         for field in extracted_info_dict:
# #             if extracted_info_dict[field].strip().lower() == 'unknown':
# #                 additional_info = input(f"Please provide the {field}: ")  # Replace with appropriate mechanism to get user input within the API
# #                 user_input += f" {field}: {additional_info}"
# #         return create_message_response(extract_information(user_input).strip())  # Retry extraction with additional info
...
#     try:
#         output = llm([{"role": "user", "content": formatted_prompt}])
...
#     print(f"Received user input: {user_input}")  # Add debugging print
...
#         print(f"Extracted info dict: {extracted_info_dict}")  # Add debugging print
...
#     Handles relative dates like 'next Friday' as well as specific dates.
#     """
#     today = datetime.now()
    
#     if 'next' in date_string.lower():
#         days = ['monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday', 'sunday']
#         target_day = next(day for day in days if day in date_string.lower())
#         days_ahead = days.index(target_day) - today.weekday()
#         if days_ahead <= 0:
#             days_ahead += 7
...
#         output = llm.invoke(formatted_prompt)
#         logging.debug(f"LLM Output: {output}")
#         extracted_info = parse_extracted_info(output.content)
        
#         # Parse and validate the completion date
#         if 'Completion Date' in extracted_info:
#             parsed_date = parse_date(extracted_info['Completion Date'])
#             if parsed_date:
#                 extracted_info['Completion Date'] = parsed_date.strftime('%Y-%m-%d')
#             else:
#                 extracted_info['Completion Date'] = 'Unknown'
...
#     try:
#         extracted_info_dict = extract_information(user_input)
